...
### 2026-05-29: globals.css traía sintaxis de Tailwind v4 sobre v3
- **Error**: El scaffold tenía `@import 'tailwindcss';` (sintaxis v4) pero `tailwindcss@3.4` + `postcss.config.js` clásico → no compila estilos
- **Fix**: Usar las directivas v3 `@tailwind base; @tailwind components; @tailwind utilities;`
- **Aplicar en**: Cualquier proyecto nuevo antes de escribir estilos (verificar versión instalada de Tailwind)
